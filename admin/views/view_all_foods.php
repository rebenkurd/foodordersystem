  ...
  <div class="main_container main_menu_open">
    ...
    <div class="home_pass hidden-xs">
      <ul>
        ...
        <li class="bring_right"><a href="manage_foods.php?source=view_all_foods&page=1">پێشاندانی هەموو خواردنەکان</a></li>
      </ul>
    </div>

    <!--/End system bath-->
    <div class="page_content">
      <h1 class="heading_title">پێشاندانی هەموو خواردنەکان</h1>
      <!--Start status alert-->
      <?php
      echo SuccessMessage();
      echo ErrorMessage();
              ...
              <tfoot>
                <tr>
                  ...
                  <td colspan="10" class="text-center">
                    <button name="deleteall" type="submit" class="btn btn-danger">سڕینەوەی هەمووی</button>
                  </td>
                </tr>
              </tfoot>
              <tbody>
                ...
                if (isset($_POST['deleteall'])) {
                  if (isset($_POST['sel'])) {
                    foreach ($_POST['sel'] as $sel) {
                      $query = "UPDATE tb_food SET recycle='1' WHERE id='$sel'";
                ...
                if (isset($_GET["page"])) {
                  ...
                  $query = "SELECT * FROM tb_food WHERE recycle='0' LIMIT $ShowPage , 5";
                  $sql = mysqli_query($con, $query);
                } else {
                ...
                ?>
                    <tr>
                      ...
                      <td>
                        <a href="manage_foods.php?source=update_food&id=<?php echo htmlentities($Id); ?>" class="fas fa-edit" data-toggle="tooltip" data-placement="top" title="گۆڕین"></a>
                        <a href="manage_foods.php?source=view_all_foods&id=<?php echo htmlentities($Id); ?>" class="fas fa-trash text-danger" data-toggle="tooltip" data-placement="top" title="سرینەوە"></a>
                      </td>
                    </tr>
                <?php
                  }
                ...
                if (isset($_GET['id'])) {
                  ...
                  $query = "UPDATE tb_food SET recycle='1' WHERE id='$Id'";
                  $sql = mysqli_query($con, $query);
                  if ($sql) {
      ...
      <nav class="text-center">
        <ul class="pagination">
          ...
          ?>
              <li>
                <a aria-label="Previous" href="manage_foods.php?source=view_all_foods&page=<?php echo htmlentities($i - 1); ?>"><span aria-hidden="true">»</span></a>
              </li>
          <?php }
          } ?>
          ...
          for ($i = 1; $i <= $PostPagination; $i++) {
            if (isset($Page)) {
          ?>
              <li><a href="manage_foods.php?source=view_all_foods&page=<?php echo htmlentities($i); ?>"><?php echo htmlentities($i); ?> <span class="sr-only">(current)</span></a></li>
          <?php
          ...
          ?>
              <li>
                <a aria-label="Next" href="manage_foods.php?source=view_all_foods&page=<?php echo htmlentities($i + 1); ?>"><span aria-hidden="true">«</span></a>
              </li>
          <?php }
          } ?>
        </ul>
