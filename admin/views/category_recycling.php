  ...
  <div class="main_container main_menu_open">
    ...
    <div class="home_pass hidden-xs">
      <ul>
        ...
        <li class="bring_right"><a href="manage_categories.php?source=category_recycling&page=1"> بەکارهێنانەوە </a></li>
      </ul>
    </div>
    ...
    <div class="page_content">
            ...
            <table>
              ...
              <tbody>
                ...
                if (isset($_POST['recoverall'])) {
                  if (isset($_POST['sel'])) {
                    foreach ($_POST['sel'] as $sel) {
                      $query1 = "UPDATE tb_category SET recycle='0' WHERE id='$sel'";
                ...
                if (isset($_POST['deleteall'])) {
                  if (isset($_POST['sel'])) {
                    foreach ($_POST['sel'] as $sel) {
                      $query2 = "DELETE FROM tb_category WHERE id='$sel'";
                ...
                if (isset($_GET["page"])) {
                  ...
                  $query = "SELECT * FROM tb_category WHERE recycle='1' LIMIT $ShowPage,5";
                  $sql = mysqli_query($con, $query);
                } else {
                  $query = "SELECT * FROM tb_category WHERE recycle='1'";
                  $sql = mysqli_query($con, $query);
                }
                ...
                ?>
                    <tr>
                      ...
                      <td>
                        <a href="manage_categories.php?source=category_recycling&id=<?php echo htmlentities($Id); ?>" class="fas fa-recycle text-success" data-toggle="tooltip" data-placement="top" title="گەڕاندنەوەی"></a>
                        <a href="views/delete_category.php?id=<?php echo htmlentities($Id); ?>&image_name=<?php echo htmlentities($Image); ?>" class="fas fa-trash text-danger" data-toggle="tooltip" data-placement="top" title="سرینەوەی بەتەواوی"></a>
                      </td>
                    </tr>
                <?php
                  }
                } else {
                  echo '<tr><td colspan="8" class="text-danger text-center" >هیچ جۆرێکی سڕاوەمان نیە</td></tr>';
                }

                if (isset($_GET['id'])) {
                  $Id = $_GET['id'];

                  $query = "UPDATE tb_category SET recycle='0' WHERE id='$Id'";
                  $sql = mysqli_query($con, $query);
                  if ($sql) {
      ...
      <nav class="text-center">
        <ul class="pagination">
          ...
          ?>
              <li>
                <a aria-label="Previous" href="manage_categories.php?source=category_recycling&page=<?php echo htmlentities($i - 1); ?>"><span aria-hidden="true">»</span></a>
              </li>
          <?php }
          } ?>
          ...
          <?php
          global $con;
          $query = "SELECT COUNT(*) FROM tb_category WHERE recycle='1'";
          $sql = mysqli_query($con, $query);
          ...
          for ($i = 1; $i <= $PostPagination; $i++) {
            if (isset($Page)) {
          ?>
              <li><a href="manage_categories.php?source=category_recycling&page=<?php echo htmlentities($i); ?>"><?php echo htmlentities($i); ?> <span class="sr-only">(current)</span></a></li>
          <?php
          ...
          ?>
              <li>
                <a aria-label="Next" href="manage_categories.php?source=category_recycling&page=<?php echo htmlentities($i + 1); ?>"><span aria-hidden="true">«</span></a>
              </li>
          <?php }
          } ?>
        </ul>
